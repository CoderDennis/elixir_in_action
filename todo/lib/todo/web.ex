defmodule Todo.Web do
  ...
  # Invoke-WebRequest -Uri "http://localhost:5454/entries?list=bob&date=20131219"
  get "/entries" do
    conn
  ...
  # Invoke-WebRequest -Method Post -Uri "http://localhost:5454/add_entry?list=bob&date=20131219&title=Dentist"
  post "/add_entry" do
    conn
    |> Plug.Conn.fetch_query_params
    |> add_entry
  ...
  defp parse_date(<<year :: binary-size(4),
                  month :: binary-size(2),
                  day :: binary-size(2)>>) do
